...
function hash(args, cb)
{
    ...
    if (args.url.match(/^http(s?):/)) return get.concat({ url: args.url, method: "HEAD" }, function(err, res, body) {
        if (err) return cb(err);

        ready((res.size = file_size = parseInt(res.headers["content-length"], 10)).toString(16), "filesize");
        var ranges = [
            { start: 0, end: chunk_size-1 },
            { start: file_size - chunk_size, end: file_size - 1 }
        ];
        function tryRange(range, next) {
            get.concat({ url: args.url, headers: { range: "bytes="+range.start+"-"+range.end, "enginefs-prio": 10 } }, function(err, res, data) {
                if (err) return cb(err);
                if (res.statusCode !== 200 && res.statusCode !== 206) return cb(new Error("non-200/206 ("+res.statusCode+") status code returned for range"))
                if (data.length !== chunk_size) return cb(new Error("response for calculating movie hash is wrong length: "+JSON.stringify(range)+" chunk_size "+chunk_size+" but received "+data.length), res);
