...
{    
	...
	*/
	...
	if (query.itemHash) subtitles.itemHash = query.itemHash;
	if (subtitles.itemHash) subtitles.id = (subtitles.itemHash || subtitles.item_hash).split(' ').join('-');
	...
	*/
	...
	async.auto({
		...
		retrieve: ["token", function(cb, res) {
			...
			var metaQuery = { imdbid: query.meta.imdb_id.slice(2), sublanguageid: null };
