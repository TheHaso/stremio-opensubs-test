...
var getAgent = function() {
	...
	if (PROXY) {
		...
		return new Socks5ClientHttpAgent({ socksPort: PROXY.port, socksHost: PROXY.host });
	}
};
...
// remove yify
//function yifyRequest(meta, callback)
//{
//	var urls = ["http://api.yifysubtitles.com/subs/", "http://api.ysubs.com/subs/"];
//	var prefixUrl = "http://www.yifysubtitles.com";

//	var response;
//	async.eachSeries(urls, function(url, cb) {
//		needle.get(url+meta.imdb_id, { json: true }, function(err, resp, data) {
//			if (err) console.error(err);
//			if (data && !data.success) console.error(data);
//			if (data && data.success) { response = data; cb(true) }
//			else cb(); // go to the next
//		})
//	}, function() {
//		if (! (response && response.success)) return callback(); // not fatal
//		if (! (response.subs && response.subs[meta.imdb_id])) return callback(); // consider logging that
//
//		callback();
//		var res = {};
//		_.each(response.subs[meta.imdb_id], function(val, key) {
//			if (! LANG_MAP[key]) { console.error("no language found in map "+key); return; }
//
//			var sub = _.max(val, function (s) { return s.rating });
//			res[LANG_MAP[key]] = { url: prefixUrl + sub.url, id: "yify-"+sub.id };
//		});
//
//		callback(null, res);
//	});
...
function findSubtitles(args, callback)
{
	...
	//var langs = query.languages.join(","); 
	...
	async.auto({
		...
		do_grouping: ["pick", function(cb, res) {
			...
			subGrouping(toGroup, function(err, groups) {
				...
				cb();
			}, { sensitivity: SUB_GROUPING_SENSITIVITY, agent: getAgent() });
