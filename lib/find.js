...
{    
	...
	*/
	...
	if (group && group[1] > res.length * 0.75) dominantGroup = group[0];
	...
	debugLog("subtitles-crawler: found from opensubtitles "+_.size(langRes)+" sets of subtitles, dominant group: "+dominantGroup);
	...
	*/
	...
	async.auto({
		...
		retrieve: ["token", function(cb, res) {
			...
			var hashQuery = { moviebytesize: parseInt(query.movieByteSize), moviehash: query.movieHash, sublanguageid: null };
